...
class Program
{
    ...
    static string plikGrafiku = "grafik_rezerwacji.csv";
    ...
    static void Main(string[] args)
    {
        ...
        ZaktualizujStanowisko(wybraneStanowisko, czyDzien, liczbaOsob);

        ZapiszDane();

        ZapiszDoGrafiku(imieNazwisko, czyDzien, wybraneStanowisko, liczbaOsob);
        ...
        Console.WriteLine();
        Console.WriteLine($"Zmiany zapisano w '{plikDanych}', a wpis dodano do '{plikGrafiku}'.");
        Console.WriteLine();
    ...
    static void ZaktualizujStanowisko(int numerStanowiska, bool czyDzien, int zarezerwowaneOsoby)
    {
        Stanowisko s = stanowiska.Find(x => x.Numer == numerStanowiska);
        if (s == null) return;

        if (czyDzien)
        {
            s.MiejscaDzien -= zarezerwowaneOsoby;
            if (s.MiejscaDzien <= 0)
            {
                s.MiejscaDzien = 0;
                s.DostepneDzien = false;
            }
        }
        else
        {
            s.MiejscaNoc -= zarezerwowaneOsoby;
            if (s.MiejscaNoc <= 0)
            {
                s.MiejscaNoc = 0;
                s.DostepneNoc = false;
    ...
    static void ZapiszDane()
    {
        ...
        linie.Add("# Łowisko Okoń - Dane stanowisk");
        linie.Add("# Format: StanowiskoNr;MiejscaDzien;DostepneDzien;MiejscaNoc;DostepneNoc");

        foreach (var s in stanowiska)
        {
            string dzien = s.DostepneDzien ? "tak" : "nie";
            string noc = s.DostepneNoc ? "tak" : "nie";
            linie.Add($"{s.Numer};{s.MiejscaDzien};{dzien};{s.MiejscaNoc};{noc}");
        }

        File.WriteAllLines(plikDanych, linie);
    }

    static void ZapiszDoGrafiku(string imieNazwisko, bool czyDzien, int numerStanowiska, int liczbaOsob)
    {
        bool plikIstnieje = File.Exists(plikGrafiku);

        using (StreamWriter sw = new StreamWriter(plikGrafiku, append: true))
        {
            if (!plikIstnieje)
            {
                sw.WriteLine("DataCzas;ImieNazwisko;Pora;Stanowisko;LiczbaOsob");
            }

            string pora = czyDzien ? "Dzień" : "Noc";
            string wiersz = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss};{imieNazwisko};{pora};{numerStanowiska};{liczbaOsob}";
            sw.WriteLine(wiersz);
        }
    }
}
